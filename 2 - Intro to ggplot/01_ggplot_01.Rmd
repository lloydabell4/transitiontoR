...
What if we wanted to learn more about those those outliers that we identified above? We can use some basic data transformation functions to quickly filter our data to learn more about those specific episodes.
...
This brings up another issue: some 'titles' are made up of two episodes and when points overlap on our visualization, we can't see them. How could we fix this? How could you explore how the role of Director impacted both imdb rating and views?
...
second_viz <- ggplot(data = title_friends, mapping = aes(x = mean_views, y = mean_rating))+
  geom_point(alpha = .5)+
   geom_text_repel(data = subset(title_friends, mean_views > 40),
                                         mapping = aes(label = title))+
  labs(title = "Friends Episodes: IMDB Rating vs Viewers", 
